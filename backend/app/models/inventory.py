...
class ResponsableIt(Base):
    __tablename__ = "responsable_it"

    id_responsable: Mapped[int] = mapped_column(Integer, primary_key=True)
    nom_responsable: Mapped[str] = mapped_column(String(150), nullable=False)
    id_auth: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), nullable=True)

    appareils: Mapped[List["Appareil"]] = relationship(back_populates="responsable_it")
...
class Utilisateur(Base):
    ...
    appareils: Mapped[List["Appareil"]] = relationship(back_populates="utilisateur")
...
class Appareil(Base):
    ...
    id_utilisateur: Mapped[int] = mapped_column(
        Integer, ForeignKey("utilisateur.id_utilisateur"), nullable=False
    )
    id_responsable: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("responsable_it.id_responsable"), nullable=True
    )

    utilisateur: Mapped["Utilisateur"] = relationship(back_populates="appareils")
    responsable_it: Mapped[Optional["ResponsableIt"]] = relationship(back_populates="appareils")
    installations: Mapped[List["Installation"]] = relationship(back_populates="appareil")
